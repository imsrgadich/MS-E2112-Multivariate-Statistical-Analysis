setwd("~/Documents/OneDrive/Aalto/Sem2/MSA/Project")

library(ggplot2)
library(e1071)
library('MASS')

cancer <- read.csv("BreastCancerData.csv",header = T,sep = ",")
...
for (i in 1:nvar) {
  ...
  hist(x[,i],main=name,xlab=NULL,ylab=NULL) }  #draw the histograms for each variable
on.exit(par(old.par))   #set the graphic parameters back to the original
}
multi.hist(data)
#############################################################################
xx <- with(cancer, data.table(id=1:nrow(cancer), group= Class,I0, PA500, HFS, DA, Area, 
                            A.DA, Max.IP, DR, P))
